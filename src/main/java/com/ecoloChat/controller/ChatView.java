...
public class ChatView extends SpringBeanAutowiringSupport implements Serializable {
	...
	public void chargerLesDiscussion(String user1){
		//System.out.println(connectedUser.getIdUtilisateur()+ "----"+UtilisateurService.getUserByEmail(user1).getIdUtilisateur());
		try {
			lMsg=UtilisateurService.getAllMsgByIdEmeteurAndRecepteur(connectedUser.getPseudo(),UtilisateurService.getUserByEmail(user1).getPseudo());
	...
	public void login() {
		...
		if(connectedUser!=null){
			...
			loggedIn = true;
			username=connectedUser.getPseudo();
			/**
			List<Utilisateur> lu=UtilisateurService.getListeAllUsers();
			for (Utilisateur ut : lu) {
				if(ut.getEmailUtilisateur()!=u.getEmailUtilisateur()){
					users.add(ut.getEmailUtilisateur());
					requestContext.execute("PF('subscriber').connect('/" + ut.getEmailUtilisateur() + "')");
				}
			}*/
			
		}else{
			FacesContext.getCurrentInstance().addMessage(null,
					new FacesMessage(FacesMessage.SEVERITY_ERROR, "Login ou mot de passe Incorrect", "Veuillez saisir les bons parametre"));
